...
export async function maybeDeslopStagedChanges(
	...
	// Handle CLI override (--deslop yes|no|<instructions>)
	if (options.deslopOverride !== undefined) {
		const override = options.deslopOverride.trim().toLowerCase();
		if (override === "no" || override === "n") {
			return "continue";
		} else if (override === "yes" || override === "y") {
			shouldDeslop = true;
		} else {
			// Treat as custom instructions
			shouldDeslop = true;
			extraPrompt = options.deslopOverride.trim();
		}
	} else if (options.yes) {
