...
async function runOpencodePrompt(
	...
	try {
		...
		if (!response.data) {
			await close();
			throw new Error(`No response from ${modelID}`);
		}

		const parts = response.data.parts ?? [];
		const message = extractTextFromParts(parts);

		if (!message) {
			await close();
			throw new Error(
				`No response generated by ${modelID}. Response: ${JSON.stringify(response.data)}`,
			);
		}
